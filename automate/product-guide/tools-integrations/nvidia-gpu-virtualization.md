...
The NVIDIA integration with VergeOS enables GPU acceleration for virtual machines through NVIDIA's vGPU and Multi-Instance GPU (MIG) technologies. Multiple VMs can share physical GPU resources while maintaining performance isolation, managed through VergeOS [resource groups](https://app.gitbook.com/s/null/product-guide/system/device-pass-overview#resource-groups) and policies alongside traditional VMs.
...
- **Tenant Isolation**: Hardware-level separation and performance guarantees between GPU workloads, with support for [passing vGPU devices to tenants](https://app.gitbook.com/s/null/product-guide/system/nvidia-vgpu-configuration#share-an-nvidia-vgpu-to-a-tenant)
- **Dynamic Allocation**: Scale GPU resources based on workload demands and priorities

!!! tip "Full GPU Passthrough"
    If your workload requires exclusive access to an entire physical GPU (e.g., for maximum performance or unsupported vGPU hardware), consider [PCI device passthrough](https://app.gitbook.com/s/null/product-guide/system/generic-pci-passthrough) instead of vGPU.

## Implementation Resources

### Getting Started

1. Review [Device Passthrough Overview](https://app.gitbook.com/s/null/product-guide/system/device-pass-overview) for foundation concepts
2. Follow the [NVIDIA vGPU Configuration](https://app.gitbook.com/s/null/product-guide/system/nvidia-vgpu-configuration) guide for step-by-step setup

### Advanced Configuration

- [VM Best Practices](https://app.gitbook.com/s/null/product-guide/virtual-machines/vm-best-practices) — Performance optimization guidelines
- [Maintenance Mode](https://app.gitbook.com/s/null/product-guide/operations/maintenance-mode) — Required when rebooting nodes for driver installation
