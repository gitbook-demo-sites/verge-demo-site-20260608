...
This method allows for the import of many VMs at once. It does not require uploading any files to the vSAN, but rather, allows for pulling data from an NFS or CIFS share. Note: For production, live, VMware environments, it is best to utilize the VMware Service to [**Import from a VMware Backup Job**](https://app.gitbook.com/s/XSPACE_RUN/product-guide/virtual-machines/import-from-vmware)

!!! warning "Import should be performed from VMs that are powered down."

!!! info "NAS Import is only for VMs, not disks. This process is for VMX or OVF files only. Any other formats will need to [Importing VMs from Files](https://docs.verge.io/knowledge-base/import-vms-from-files/)."

## Import from Volume

1. To utilize an external NFS or CIFS share for VM import, the external source must be set up as a **Remote Volume** in the NAS. See [**Remote Volumes**](https://app.gitbook.com/s/XSPACE_RUN/product-guide/nas/nas-remote-volumes) for instructions.
...
11. ***Preferred Tier*** can be selected or left at **--default--**. This determines the tier first attempted for VM storage. The [**Preferred Tiers**](https://app.gitbook.com/s/XSPACE_RUN/product-guide/storage/preferred-tiers) page provides a detailed explanation of Preferred Tier.
...
See [**Viewing Import Jobs**](https://app.gitbook.com/s/XSPACE_RUN/product-guide/virtual-machines/view-import-jobs) for more information on viewing the details of an import job.
