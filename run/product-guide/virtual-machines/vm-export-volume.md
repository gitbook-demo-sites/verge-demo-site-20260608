...
- To make the exported data accessible to external systems, such as third-party backup tools or external storage platforms:
    * The volume can be [shared over CIFS or NFS](https://app.gitbook.com/s/XSPACE_RUN/product-guide/nas/nas-shares)
    * Data can be [volume synchronized](https://app.gitbook.com/s/XSPACE_RUN/product-guide/nas/volume-syncs) to an external system (e.g. NAS appliance) via CIFS/NFS using a mounted [remote volume](https://app.gitbook.com/s/XSPACE_RUN/product-guide/nas/nas-remote-volumes)
