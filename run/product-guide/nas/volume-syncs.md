...
Volume syncs allow for synchronizing data between two volumes. A volume sync can be used as a one-time transfer or recurring in order to synchronize data on a regular schedule. Volume syncs can involve both [**Remote Volumes**](https://app.gitbook.com/s/null/product-guide/nas/nas-remote-volumes) and [**Local Volumes**](https://app.gitbook.com/s/null/product-guide/nas/nas-local-volumes), providing the ability to:
