...
- Physical disks in VergeOS Systems vSAN [**Tier 0**](https://app.gitbook.com/s/null/product-guide/storage/storage-tiers#tier-0-metadata-tier)
...
!!! tip "Deduplication"
    VergeOS vSAN handles data deduplication natively at the block level. When using external storage with vSAN, you should disable deduplication on your SAN if it does not support cross-LUN deduplication. If your SAN supports global deduplication, we recommend leaving it enabled to reduce overall storage consumption. VergeOS replicates data across FC LUNs according to the system's configured [vSAN redundancy level](https://app.gitbook.com/s/null/product-guide/storage/vsan-redundancy-levels) (N+1 stores 2 copies; N+2 stores 3 copies).
...
!!! warning "Use Maintenance Mode"
    Always enter [**Maintenance Mode**](https://app.gitbook.com/s/null/product-guide/operations/maintenance-mode) before modifying storage configurations.
...
- [Maintenance Mode](https://app.gitbook.com/s/null/product-guide/operations/maintenance-mode)  
- [System Monitoring](https://app.gitbook.com/s/null/product-guide/system/subscriptions-overview)
