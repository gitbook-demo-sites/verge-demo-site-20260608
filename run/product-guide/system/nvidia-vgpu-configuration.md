...
- **IOMMU / VT-d / SR-IOV enabled** in the host BIOS — if not yet enabled, plan for a node reboot during this process (follow proper [**Maintenance Mode**](https://app.gitbook.com/s/null/product-guide/operations/maintenance-mode) procedures).
...
- Familiarity with [**PCI Passthrough Risks and Precautions**](https://app.gitbook.com/s/null/product-guide/system/device-pass-overview#pci-passthrough-risksprecautions)
...
After the resource group is selected or a new one is created, a **Success** message should appear indicating resource rules were created for the device(s).
   * If a driver has not been previously loaded or IOMMU is not yet enabled, **reboot the associated node(s)** in [**Maintenance Mode**](https://app.gitbook.com/s/null/product-guide/operations/maintenance-mode) before continuing.
...
* When resource group configuration is complete and has been submitted, place the node into [**Maintenance Mode**](https://app.gitbook.com/s/null/product-guide/operations/maintenance-mode) and click **Reload Drivers**.
...
!!! tip
    The resource group dashboard contains the resource rules that were auto-generated based on your selected NVIDIA devices. You can click an individual rule to view configuration detail. A system-created rule can be modified as needed. For example, the *Node* filter can be changed to *-- None --* to include matching devices from all nodes; the *slot* filter can be removed or modified to accommodate devices that may reside on different slots within the same node or across different nodes. Information regarding resource rules is available at: [**Device Passthrough Overview - Resource Rules**](https://app.gitbook.com/s/null/product-guide/system/device-pass-overview#resource-rules)
