...
See [**Increasing a Tenant's Resources**](https://app.gitbook.com/s/null/product-guide/tenants/add-tenant-resources) or [**Reducing a Tenant's Resources**](https://app.gitbook.com/s/null/product-guide/tenants/reduce-tenant-resources) for instructions regarding modifying resources.
...
* **UI Address** - Link for the tenant to access their own VergeOS UI. This field is **autopopulated** with the [**first External IP**](https://app.gitbook.com/s/null/product-guide/tenants/assign-ip-to-tenant) assigned to the tenant. If/when additional external IPs are assigned to a tenant, the **UI address can be changed** to one of the other IP addresses. Rules will **automatically** be updated by the system; *Apply Rules* to **both** the associated **External** network and **tenant** network if *UI Address* is modified.
